...
export default class CreateUsers implements Seeder {
  public async run(factory: Factory): Promise<void> {
    await factory(User)()
      .map(async (user: User) => {
        const comments: Comment[] = await factory(Comment)()
        ...
        const likes: Like[] = await factory(Like)()
          .map(async (like: Like) => {
        ...
        const userPosts: Post[] = await factory(Post)()
          .map(async (post: Post) => {
            post.comments = comments;
            post.likes = likes;
            post.latestComment = await factory(Comment)()
              .map(async (comment: Comment) => {
                comment.author = await factory(User)().create();
                return comment;
              })
        ...
        const notifications: Notification[] = await factory(Notification)()
